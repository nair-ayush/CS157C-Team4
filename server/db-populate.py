...
def insert_dummy_data():
    ...
    with flaskapp.app_context():
        ...
        df = pd.read_json('data/outdoor_activities.json')

        if 'created_by' in df:
            del df['created_by']

        cities = ["New York", "Los Angeles", "San Francisco", "San Jose", "San Diego", "San Francisco"]
        ...
        events_df = pd.read_json('data/outdoor_activities.json')
        landmarks_df = pd.read_json('data/landmarks.json')
        restaurants_df = pd.read_json('data/eateries.json')
        
        listing = ["Stylish, Spacious and Quiet Flat", "Downtown Apt", "Beach House", "Luxury Tree House", "Villa in Houston", "Bright, Modern Garden Unit - 1BR/1BTH", "Classic Nob Hill Studio - Roof Deck"] 

        for i in range(6):
            location = choice(cities)
            start_date = datetime.now() + timedelta(days= randint(1, 60))
            end_date = (start_date + timedelta(days= randint(1, 30)))
            stay = choice(listing)

            random_restaurants = restaurants_df.sample(n=3, random_state=1)
            random_events = events_df.sample(n=3, random_state=1)
            random_landmarks = landmarks_df.sample(n=3, random_state=1)

            budget = randint(10, 200)
            activities = random_restaurants['name'].tolist() + random_events['name'].tolist() + random_landmarks['name'].tolist()
